(function ($) {
   ...
   /* var runningAnimationFrame = true;
    ...
    function getLastGames() {
        ...
        setFilterParam({type: 'last',games: $.cookie('last_games').split(',')});
        applyFilters();
    ...
    function search(elm) {
        ...
        if (elm.val().length > 0) {
            setFilterParam({type: 'search', search: elm.val()});
        } else {
            emptySearch();
	...
	*/
	...
	$(document).on('click','.js-game-like',function(){
		...
		{			
				...
				$.post('/games/add-to-fav',{ game_id : gameId },function(res){
        ...
        if (games.result.length > 0) {
            ...
            $.each(games.result,function(index,value){
                ...
                if (!logged) {
                    ...
                    game_item += '<a href="'+(mobile ? value.iframe_logged : '#')+'" data-has-demo="'+value.has_demo+'" class="game-link js-open-game" data-game-id="' + value.id + '" data-src="' + value.iframe_logged + '" style="background-image: url(' + value.game_img + ');">';
                }
